  ...
  return dispatch => {
    ...
    axios
      ...
      .then(response => {
        ...
        const generateRefundLabel = new GenerateRefundLabel(deliveryAddress, name, data.refundNumber, data.refundId, data.customer.email, orderNumber);
...
export const createRefundLabel = (request) => {
  return dispatch => {
    axios
      .post(
        NODE_API_URL + REFUND_BASE_URL + "/generateRefundPDF",
        request
      )
      .then(({ data }) => {
...
export const sendRefundLabel = (request) => {
  return dispatch => {
    axios
      .post(
        NODE_API_URL + EMAIL_BASE_URL + "/sendRefundLabel",
        request
      )
      .then(({ data }) => {
